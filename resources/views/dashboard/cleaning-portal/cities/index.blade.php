...
<p style="font-size:15px;color:var(--text-mid);line-height:1.6;max-width:720px;margin-bottom:20px">List every city and country where your crew takes turnover and cleaning jobs. Landlords find you by property location — add all areas you cover, and mark one as <strong>primary</strong> for your public profile.</p>
<div class="clean-grid-2" style="align-items:start">
  ...
  @if($editingCity)
    <div class="db-card-header" style="flex-wrap:wrap;gap:8px">
      ...
      <a href="{{ route('clean.cities.index') }}" class="db-btn db-btn-ghost" style="font-size:13px;text-decoration:none">Cancel</a>
    </div>
    <div class="db-card-body">
      <form method="POST" action="{{ route('clean.cities.update', $editingCity) }}" class="db-form">
        @csrf @method('PUT')
  ...
  @else
    ...
    <div class="db-card-body">
      <form method="POST" action="{{ route('clean.cities.store') }}" class="db-form">
        @csrf
        <div class="db-form-row">
        ...
        <table class="db-table">
          ...
          <tbody>
            @forelse($cities as $city)
              <tr @if($editingCity && $editingCity->id === $city->id) style="background:var(--green-pale)" @endif>
                ...
                <td style="white-space:nowrap">
                  <a href="{{ route('clean.cities.index', ['edit' => $city->id]) }}" class="db-table-link">Edit</a>
                  @if($cities->count() > 1)
                    <form method="POST" action="{{ route('clean.cities.destroy', $city) }}" style="display:inline;margin-left:10px" onsubmit="return confirm('Remove {{ $city->city }} from coverage?')">
                      @csrf @method('DELETE')
