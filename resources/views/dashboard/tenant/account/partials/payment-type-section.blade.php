...
<section class="rm-pm-section" id="pm-{{ $type }}">
        ...
        <tbody>
          ...
          <tr>
            ...
            <td style="white-space:nowrap">
              <a href="{{ route('tenant.account', array_merge($pmQuery, ['edit' => $pm->id])) }}" class="db-table-link">Edit</a>
              @if(! $pm->is_default)
                ·
                <form method="POST" action="{{ route('tenant.account.payment-methods.default', $pm) }}" style="display:inline">
                  @csrf @method('PATCH')
              ...
              @endif
              ·
              <form method="POST" action="{{ route('tenant.account.payment-methods.destroy', $pm) }}" style="display:inline" onsubmit="return confirm('Remove this method?');">
                @csrf @method('DELETE')
  ...
  <div class="db-card">
    <div class="db-card-header">
      ...
      @if($isEditing)
        <a href="{{ route('tenant.account', $pmQuery) }}" class="db-btn db-btn-ghost" style="font-size:12px;padding:5px 10px;text-decoration:none">Cancel</a>
      @endif
    </div>
    <div class="db-card-body">
      <form method="POST"
        action="{{ $isEditing ? route('tenant.account.payment-methods.update', $editingMethod) : route('tenant.account.payment-methods.store') }}"
        class="db-form" style="max-width:none">
        @csrf
        ...
        @if($type === 'card')
          ...
          <div class="db-form-row">
            ...
            <div class="db-form-group">
              <label>Label</label>
              <input type="text" name="label" class="db-input" maxlength="120" value="{{ old('label', $isEditing ? $editingMethod->label : '') }}" placeholder="Personal Visa">
            </div>
          </div>
          @include('dashboard.tenant.account.partials.card-billing-fields', ['user' => $user, 'isEditing' => $isEditing, 'editingMethod' => $editingMethod])
        @elseif($type === 'ach')
