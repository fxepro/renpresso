...
@section('page-title', 'Maintenance team')

@section('topbar-actions')
  <button type="button" class="db-btn db-btn-ghost" onclick="document.getElementById('invitePanel').classList.toggle('open')">Invite team</button>
@endsection
...
<div id="invitePanel" class="mt-invite-panel {{ session('show_invite_panel') || $errors->has('email') ? 'open' : '' }}" onclick="if(event.target===this)this.classList.remove('open')">
  <div class="mt-invite-sheet" onclick="event.stopPropagation()">
    <h3>Invite a team you know</h3>
    <p class="db-form-hint" style="margin-bottom:16px">For teams you already work with. They register with this email and join your roster automatically.</p>
    <form method="POST" action="{{ route('landlord.maintenance-team.invite') }}" class="db-form" style="max-width:none">
      @csrf
      <div class="db-form-group">
        <label for="invite_email">Email</label>
        <input type="email" class="db-input" id="invite_email" name="email" value="{{ old('email') }}" required placeholder="team@example.com">
...
<div class="db-card">
  <nav class="mt-tabs" aria-label="Maintenance teams">
    <a href="{{ route('landlord.maintenance-team.index', ['tab' => 'roster']) }}"
       class="mt-tab {{ $activeTab === 'roster' ? 'active' : '' }}">
      Your teams
      @if($myTeams->isNotEmpty())<span class="mt-tab-count">{{ $myTeams->count() }}</span>@endif
    </a>
    <a href="{{ route('landlord.maintenance-team.index', ['tab' => 'discover']) }}"
  ...
  <div class="mt-tab-panel {{ $activeTab === 'roster' ? 'active' : '' }}" id="tab-roster">
    <div class="db-card-body">
      <p class="db-form-hint" style="margin-bottom:16px">Teams you added from <strong>Search &amp; add</strong>. Assign them on <a href="{{ route('maintenance.index') }}" class="db-table-link">Maintenance requests</a> so they receive tenant jobs and can update status.</p>
      @if($myTeams->isEmpty())
        <div class="db-empty" style="padding:32px 0">
          <div class="db-empty-icon">🧰</div>
          <h3>No teams on your roster yet</h3>
          <p>Switch to <strong>Search &amp; add</strong> to find teams in your property cities.</p>
          <a href="{{ route('landlord.maintenance-team.index', ['tab' => 'discover']) }}" class="db-btn db-btn-primary" style="margin-top:12px">Search &amp; add</a>
        </div>
      @else
        @include('dashboard.maintenance-team.partials.teams-table', [
          'teams' => $myTeams,
          'showRemove' => true,
          'showEngage' => false,
        ])
    ...
    <div class="db-card-body">
      @if($propertyLocations->isEmpty())
        <div class="db-empty" style="padding:32px 0">
          ...
          <p>We list maintenance teams in the same city and country as your properties.</p>
      ...
      @else
        ...
        <form method="GET" action="{{ route('landlord.maintenance-team.index') }}" class="mt-search">
          ...
          @if($search || $cityFilter)
            <a href="{{ route('landlord.maintenance-team.index', ['tab' => 'discover']) }}" class="db-btn db-btn-ghost">Clear</a>
          @endif
        </form>

        @if($browseTeams->isEmpty())
          <p class="db-form-hint" style="margin:24px 0 0">No teams match — you may already have them on your roster. Try clearing filters or use <strong>Invite team</strong>.</p>
        @else
          @include('dashboard.maintenance-team.partials.teams-table', [
            'teams' => $browseTeams,
            'showRemove' => false,
