...
<div class="db-table-wrap">
  <table class="db-table mt-teams-table">
    ...
    <tbody>
      ...
      <tr>
        <td>
          <a href="{{ route('landlord.maintenance-team.show', $team) }}" class="db-table-link">{{ $team->name }}</a>
          @if($team->owner)
        ...
        <td class="mt-td-rating">
          @if($count > 0)
            @include('dashboard.maintenance-team.partials.rating-stars', ['rating' => $avg, 'size' => 'sm', 'showValue' => true])
        ...
        <td class="mt-td-actions">
          <div class="mt-row-actions">
            <a href="{{ route('landlord.maintenance-team.show', $team) }}" class="db-btn db-btn-ghost" style="padding:6px 12px;font-size:14px">View</a>
            @if($showRemove)
