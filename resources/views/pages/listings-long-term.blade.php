@php
  ...
  $listingIndexUrl = route('listings.long-term');
@endphp
@extends('layouts.marketing', ['page' => 'listings'])

@section('title', 'Long-term listings')
@section('meta_description', 'Browse long-term rental listings from landlords on Renpresso.')

@section('content')

@include('partials.sections.marketing-hero', [
  'label' => 'Directory',
  'title' => 'Long-term <em>rentals</em>',
  'lead' => 'Public listings from landlords using ' . config('app.name') . ' for monthly leases. Filter by country or search by city or name.',
  'ctas' => [
    ['href' => route('listings.short-term'), 'label' => 'Short-term stays →', 'class' => 'btn-outline-light'],
  ...
  <div class="container">
    ...
    @if ($properties->isEmpty())
      <div class="lc-empty">
        ...
        <p class="lc-empty-title">No public long-term listings match your filters yet.</p>
        <p class="lc-empty-sub">Landlords can set a property to <strong>public</strong> and <strong>long-term</strong> in the dashboard to appear here.</p>
      </div>
    @else
      <div class="lc-grid">
        @foreach ($properties as $prop)
          @php
            ...
            $bdHtml = $prop->bedrooms !== null
              ...
              : '';
          @endphp
          <a class="prop-card" href="{{ route('listings.long-term.show', $prop) }}">
            {!! $coverHtml !!}
            <div class="prop-card-top">
              <span class="prop-card-flag">{{ $flag }}</span>
              <span class="lc-badge lc-badge-long">Long-term</span>
            </div>
