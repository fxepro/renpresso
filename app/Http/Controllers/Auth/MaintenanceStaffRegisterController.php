...
class MaintenanceStaffRegisterController extends Controller
{
    public function show(?string $invite_token = null)
    {
        ...
        $invite = null;
        if ($invite_token) {
            $invite = MaintenanceStaffInvite::query()->where('token', $invite_token)->first();
            if (! $invite || ! $invite->isUsable()) {
                return view('pages.register-maintenance', [
                    'invite'       => null,
                    'invite_token' => null,
                    'inviteError'  => 'This invite link is invalid or has expired. You can still register your team below — landlords in your city can find you in the directory.',
                ]);
            }
        }
    ...
    public function store(Request $request)
    {
        ...
        if ($request->filled('invite_token')) {
            $invite = MaintenanceStaffInvite::query()->where('token', $request->input('invite_token'))->first();
            if (! $invite || ! $invite->isUsable()) {
        ...
        if ($invite) {
            $invite->landlord->engagedMaintenanceTeams()->syncWithoutDetaching([$team->id]);
            $invite->update([
                'used_at'       => now(),
        ...
        $message = $invite
            ? 'Your team is live and connected to '.$invite->landlord->fullName().'.'
            : 'Your team is listed in '.$team->locationLabel().'. Landlords with properties there can add you to their team.';

        return redirect()->route('maint.dashboard')->with('success', $message);
    }
}
